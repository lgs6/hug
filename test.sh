...
# 是否开启 Hugging Face API 自动保活，可选，填 "true" 或 "false"
# 如果开启，请务必填写下面的 HF_TOKEN_INPUT 和 HF_REPO_ID_INPUT
KEEP_ALIVE_HF="true"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}    Python Xray Argo 一键部署脚本 (固定配置版)    ${NC}"
echo -e "${GREEN}========================================${NC}"
echo
...
echo
echo -e "${GREEN}本脚本已修改为固定参数配置模式${NC}"
echo -e "${GREEN}配置信息已从脚本顶部读取，无需手动输入${NC}"
echo
...
# 使用脚本顶部的配置信息修改 app.py
echo -e "${BLUE}=== 应用固定配置 ===${NC}"

# 如果 UUID_INPUT 为空，则自动生成
...
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    if [ -n "$HF_TOKEN_INPUT" ] && [ -n "$HF_REPO_ID_INPUT" ]; then
        echo -e "${GREEN}Hugging Face API 保活已设置！${NC}"
        echo -e "${GREEN}目标仓库: $HF_REPO_ID_INPUT${NC}"
    else
        echo -e "${RED}错误：开启保活需同时设置 HF_TOKEN 和 HF_REPO_ID。已取消保活。${NC}"
        KEEP_ALIVE_HF="false"
    fi
fi
...
echo
echo -e "${GREEN}固定配置完成！${NC}"

# ... 后续脚本内容保持不变 ...
# (此处省略与原脚本相同的服务启动、分流配置、节点生成和信息显示等部分)
# (为了简洁，只显示了配置部分的核心修改)
...
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    ...
    echo "    status_code=\$(curl -s -o /dev/null -w \"%{http_code}\" --header \"Authorization: Bearer $HF_TOKEN_INPUT\" \"https://huggingface.co/api/spaces/$HF_REPO_ID_INPUT\")" >> keep_alive_task.sh
    echo "    if [ \"\$status_code\" -eq 200 ]; then" >> keep_alive_task.sh
    echo "        echo \"Hugging Face API 保活成功 (Space: $HF_REPO_ID_INPUT, 状态码: 200) - \$(date '+%Y-%m-%d %H:%M:%S')\" > keep_alive_status.log" >> keep_alive_task.sh
    echo "    else" >> keep_alive_task.sh
    echo "        # 尝试 Models API" >> keep_alive_task.sh
    echo "        status_code_model=\$(curl -s -o /dev/null -w \"%{http_code}\" --header \"Authorization: Bearer $HF_TOKEN_INPUT\" \"https://huggingface.co/api/models/$HF_REPO_ID_INPUT\")" >> keep_alive_task.sh
    echo "        if [ \"\$status_code_model\" -eq 200 ]; then" >> keep_alive_task.sh
    echo "            echo \"Hugging Face API 保活成功 (Model: $HF_REPO_ID_INPUT, 状态码: 200) - \$(date '+%Y-%m-%d %H:%M:%S')\" > keep_alive_status.log" >> keep_alive_task.sh
    echo "        else" >> keep_alive_task.sh
    echo "            echo \"Hugging Face API 保活失败 (仓库: $HF_REPO_ID_INPUT, Space API状态: \$status_code, Model API状态: \$status_code_model) - \$(date '+%Y-%m-%d %H:%M:%S')\" > keep_alive_status.log" >> keep_alive_task.sh
